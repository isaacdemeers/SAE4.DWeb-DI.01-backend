...
export default function Card({ data }) {
  ...
  return (
    <Link to={'/viewMovie/' + data['id']}>
      ...
      <figure className="card group flex relative items-center justify-center flex-col  w-[fit-content] rounded-2xl">
        ...
        <figcaption className="text-light text-sm p-3 text-center max-w-[18vw] whitespace-nowrap overflow-hidden overflow-ellipsis min-w-[200px]">{data['name']}</figcaption>

        <section className="cardPreview group-hover:scale-80 invisible bg-opacity-70 bg-dark backdrop-blur-lg shadow-xl md:flex z-50 justify-start items-start flex-col overflow-hidden h-[fit-content] rounded-2xl w-[350px] [transition:all_.3s] absolute top-2/4 left-2/4 -translate-x-[45%] -translate-y-1/2  origin-[0_0]">
          ...
          <div className="p-[.6rem]">
            <figcaption className="text-[1.2rem] font-medium text-white">{data['name']}</figcaption>
            ...
            <figcaption className="text-[.6rem] text-light w-full mb-4">{data['description'].replace('<div>', '').replace('</div>', '')}</figcaption>
